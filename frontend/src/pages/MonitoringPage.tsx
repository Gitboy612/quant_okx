...
const EVENT_ICONS: Record<string, { icon: typeof Play; color: string; label: string }> = {
  ...
  stopped: { icon: Square, color: '#FF4757', label: '已停止' },
  ...
  order_canceled: { icon: X, color: '#6B6B7B', label: '已撤销' },
  order_failed: { icon: AlertTriangle, color: '#FF4757', label: '下单失败' },
  pnl_recorded: { icon: DollarSign, color: '#A855F7', label: '盈亏记录' },
  error: { icon: AlertCircle, color: '#FF4757', label: '错误' },
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium text-[#E8E8ED]">策略监测</h2>
      </div>
      ...
      >
        <div className="flex items-center gap-4 mb-4">
          <label className="text-xs text-[#6B6B7B] flex-shrink-0">选择策略</label>
          {instancesLoading ? (
            <div className="flex-1 h-10 bg-[#1E1E28] rounded-lg animate-pulse" />
          ) : (
            <Dropdown
          ...
          {selectedInstanceId && (
            <div className="flex items-center gap-2">
              <button
                onClick={loadEvents}
                disabled={loading}
                className="flex items-center gap-1.5 border border-[#1E1E28] text-[#6B6B7B] rounded-md px-3 py-2 text-xs hover:bg-[#1A1A24] hover:text-[#E8E8ED] transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 border border-[#1E1E28] text-[#00D4AA] rounded-md px-3 py-2 text-xs hover:bg-[#00D4AA]/10 transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 border border-[#FF4757]/20 text-[#FF4757] rounded-md px-3 py-2 text-xs hover:bg-[#FF4757]/10 transition-colors disabled:opacity-50"
              >
        ...
        {selectedInstanceId === null ? (
          <div className="flex flex-col items-center justify-center py-16 text-[#6B6B7B] text-sm gap-2">
          ...
          <div className="relative">
            <div className="absolute left-[19px] top-2 bottom-2 w-px bg-[#1E1E28]" />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 pt-1.5">
                      ...
                      <p className="text-sm text-[#E8E8ED] mt-1 leading-relaxed">{event.message}</p>
                      {event.details && (
                        <p className="text-xs text-[#6B6B7B] mt-0.5 font-mono whitespace-pre-wrap break-all">
                          {event.details}
                        </p>
                      )}
                    </div>
